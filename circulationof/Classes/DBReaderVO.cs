...
namespace Circulation
{
    class DBReader : DB
    {
        ...
        {
            DA.SelectCommand.CommandText = "select * from Reservation_R..ISSUED_OF where IDREADER = " + r.ID + " and IDSTATUS = 1";
            DS = new DataSet();
            int i = DA.Fill(DS, "t");
            if (i >= 4) return true; else return false;
        }

        internal DataTable GetFormular(int ID)//,kzzzzzzzzzzzzzzz
        {
            DA.SelectCommand.CommandText = "select ROW_NUMBER() over(order by A.DATE_ISSUE) num, " +
                                           ...
                                           " ,Reservation_R.dbo.GetProlongedTimes(A.ID, 'BJVVV') prolonged " +
                                           "  from Reservation_R..ISSUED_OF A " +
        ...
        {
            DA.SelectCommand.CommandText = "SELECT Comment FROM " + DB.BASENAME + "..Comments_OF " +
        ...
        {
            DA.SelectCommand.CommandText = "SELECT ID, Comment FROM " + DB.BASENAME + "..Comments_OF " +
                                                   " where IDReader = " + IDReader;
            DataSet DS = new DataSet();
            int t = DA.Fill(DS, "t");
            if (t == 0)
            {
                DA.InsertCommand.CommandText = "insert into " + DB.BASENAME + "..Comments_OF (IDReader, Comment) values (@IDReader, @Comment)";
                DA.InsertCommand.Parameters.Clear();
            ...
            else
            {
                ...
                DA.UpdateCommand.CommandText = " update " + DB.BASENAME + "..Comments_OF " +
                                               " set Comment = @Comment "+
                                               " where IDReader = @IDReader";
                DA.UpdateCommand.Connection.Open();
                DA.UpdateCommand.ExecuteNonQuery();
